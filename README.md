...
Replication (Master - Master) settings. Set `mysql_server_id` and `mysql_replication_role` by server (e.g. the first master would be ID `1`, with the `mysql_replication_role` of `co-master`, and the second one would be ID `2`, with the `mysql_replication_role` of `co-master`). The `mysql_replication_user` uses the same keys as individual list items in `mysql_users`, and is created on master servers, and used to replicate on all the slaves.
